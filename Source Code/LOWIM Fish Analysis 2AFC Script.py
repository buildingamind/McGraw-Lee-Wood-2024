...
# logs_dir = "/content/drive/Shareddrives/BL-INFO-BuildingAMindLab/Joshua McGraw/New FishAI Nature Experiments/UvsT Replication Study/Testing/2FAC Evaluation/"
logs_dir = "E:/EXP3_2AFC_LOWIM_CSV"
...
for model_name, model_data in models.items():
    ...
    for file_name, agent_data in model_data.items():
        agent_data = pd.read_csv(f"{logs_dir}/{file_name}", on_bad_lines='skip', nrows=500000).iloc[:, :-1]  #
        agent_name = file_name.split("_0.001")[0].replace(f"{model_name}_", "").replace("Agent_", "")
    ...
    results_df.to_csv(f"{logs_dir}/LOW_{model_name}_2AFC_SocialPreference_by_Episode.csv", index=False)
    print(f"Results Logged to {logs_dir}/LOW_{model_name}_2AFC_SocialPreference_by_Episode.csv")
    ...
    save_name = f"{logs_dir}/2AFC_LOWIM_{model_name}.png"
